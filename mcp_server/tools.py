...
def _screen_aux_field(value: str, *, field_name: str) -> str:
...
def get_disk_hash(image_path: str) -> dict[str, Any]:
    ...
    try:
        validated = DiskToolInput(image_path=image_path).image_path
        ...
        logger.log_blocked_attempt(tool="get_disk_hash", args=args, reason=str(e))
...
def get_mft_timeline(
    ...
    try:
        validated = DiskToolInput(image_path=image_path).image_path
        parsers = _screen_aux_field(parsers, field_name="parsers")
...
def get_event_logs(image_path: str, event_ids: str = "4624,4648,4672,4688") -> dict[str, Any]:
    ...
    try:
        validated = DiskToolInput(image_path=image_path).image_path
        event_ids = _screen_aux_field(event_ids, field_name="event_ids")
        result = _safe_run(
...
def extract_prefetch(image_path: str, output_format: str = "json") -> dict[str, Any]:
    ...
    try:
        validated = DiskToolInput(image_path=image_path).image_path
        output_format = _screen_aux_field(output_format, field_name="output_format")
        result = _safe_run(
...
def get_registry_key(image_path: str, hive: str) -> dict[str, Any]:
    ...
    try:
        validated = DiskToolInput(image_path=image_path).image_path
        hive = _screen_aux_field(hive, field_name="hive")
        result = _safe_run(
...
def scan_yara(image_path: str, rules_path: str = "/opt/find-evil/yara_rules") -> dict[str, Any]:
    ...
    try:
        validated = DiskToolInput(image_path=image_path).image_path
        rules_path = _screen_aux_field(rules_path, field_name="rules_path")
        result = _safe_run(
...
def run_volatility_pslist(memory_path: str, output_format: str = "json") -> dict[str, Any]:
    ...
    try:
        validated = MemoryToolInput(memory_path=memory_path).memory_path
        output_format = _screen_aux_field(output_format, field_name="output_format")
        result = _safe_run(
